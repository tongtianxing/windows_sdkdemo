...
class CButtonSTEX : public CButton
{
public:
    CButtonSTEX();
	~CButtonSTEX();
	...
	enum	{	BTNST_COLOR_BK_IN	= 0,		// Background color when mouse is INside
				...
				BTNST_COLOR_FG_ACTION,			// Text color when the button is action    add by lsn	
				BTNST_COLOR_WITER,			    // Witer of Text color     add by lsn	
				BTNST_MAX_COLORS
			};
    ...
    enum	{	BTNST_PRESSED_LEFTRIGHT = 0,	// Pressed style from left to right (as usual)
				...
				BTNST_PRESSED_NOMOVE			// Pressed style from no move
			};

	#ifdef VESION_9000
	DECLARE_DYNAMIC(CButtonSTEX)
    #endif
...
public:
	...
	// 刷新所有状态下按钮的背景色
	DWORD SetBackColor(COLORREF crColor, bool bRepaint = TRUE);
	...
	DWORD SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut = NULL, COLORREF crTransColorOut = 0, int nBitmapDown = NULL, COLORREF crTransColorDown = 0, int nBitmapDisable = NULL, COLORREF crTransColorDisable = 0);
	DWORD SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut = NULL, COLORREF crTransColorOut = 0, HBITMAP hBitmapDown = NULL, COLORREF crTransColorDown = 0, HBITMAP hBitmapDisable = NULL, COLORREF crTransColorDisable = 0);
	//Howardd 2012-12-06
	DWORD SetBitmaps( LPCTSTR lpszBmpIn, LPCTSTR lpszBmpOut, LPCTSTR lpszBmpDown, LPCTSTR lpszBmpDisable );
...
protected:
	...
	virtual void DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled,BOOL bIsAction = false, BOOL bFontNormal= TRUE, BOOL bWiter = FALSE, long lWeight = FW_LIGHT);
...
public:
	// 刷新按钮里的字体颜色
	DWORD SetTextColor_EX(BOOL bActive);
	BOOL m_bIsAction;
	// 是否为白色字体
	void SetWriteFont(bool bWrite);

	//是否使用只用生成的不可用状态贴图add by hezh
	BOOL m_bUseAutoDisableBmp;
	// 计算文本输出的位置
	void CalcTextRegion(CDC * pDC,LPCTSTR lpszText, BOOL bHCenter, BOOL bVCenter, CSize szOff, CRect &rtText);
	// 设置按钮的文本(包括指定文本的内容,字体的大小,排布的方式等)
	void SetBtnText(LPCTSTR szText, BOOL bHCenter, BOOL bVCenter, CSize szOff, LONG lWidth, LONG lHeight, COLORREF rgbOut, COLORREF rgbIn);
	// 字体
	CFont m_ftText;
	// 所有成功时才显示字体
	bool m_bDrawText;
	...
	CRect m_rtText;
	// 在指定区域内画文字
	void DrawTextInRegion(CDC * pDC, LPCTSTR lpszText, BOOL bIsAction);
	// 按钮里的字体颜色
	void SetBtnTextColor(COLORREF rgbOut, COLORREF rgbIn);
	//Howard 2012-12-13
	void SetSelected( BOOL bSelected );
private:
	CString m_strText;
	BOOL m_bSelected;	//Howard 2012-12-13
public:
	// 是否为英文版客户端
	bool m_bUseEngFont;
};
