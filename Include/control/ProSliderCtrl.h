...
public: // Progress Control functions
	...
	COLORREF _SetChColorBK(COLORREF clrNew);
	...
	void SetDrawThumb(BOOL bDrawThumb) { m_bDrawThumb = bDrawThumb; }
public:
	CProSliderCtrl();
...
protected:
	...
	void RecursiveRectEx(CDC *pDC, CRect rc, COLORREF nClr, BOOL First=FALSE);
	void RecursiveChannel(CDC *pDC, CRect rc, COLORREF nClr,
		BOOL First=FALSE, BOOL Vertical=FALSE,BYTE StepSize=20);
	void RecursiveChannelEx(CDC *pDC, CRect rc, COLORREF nClr,
		BOOL First=FALSE, BOOL Vertical=FALSE,BYTE StepSize=20);
...
public:
	...
	void DrawTranBK(LPNMCUSTOMDRAW lpcd);
