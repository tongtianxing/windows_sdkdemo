...
//RUS: 蔓疣忭桠囗桢 觐眵桕?蝮腧栾?铗眍耔蝈朦眍 屐屙蜞 戾睨
...
public:
	// 列表、树状列表用，在Item内移动时不重绘
	void SetHoldMouseMove(BOOL bHold) { m_bHoldMouseMove = bHold; }
	// 树状列表用，点击事件透传到树控件
	void SetClickRelay(BOOL bEnabled) { m_bClickRelay = bEnabled; }
	// 自动换行，列表用，限制宽度为屏幕的一半
	void SetLineWrap(BOOL bEnabled) { m_bLineWrap = bEnabled; }
};
